...
//配色资源
import { g_JSChartResource, JSCHART_LANGUAGE_ID, g_JSChartLocalization } from './umychart.resource.wechat.js';

import { IFrameSplitOperator } from './umychart.framesplit.wechat.js';

import { JSIndexScript } from './umychart.index.data.wechat.js';

import { HQ_DATA_TYPE, ChartData, HistoryData, SingleData, MinuteData } from './umychart.data.wechat.js';

import { g_MinuteCoordinateData, MARKET_SUFFIX_NAME } from './umychart.coordinatedata.wechat.js';
...
function Node() {
    ...
    this.VerifySymbolFunction = function (callee, args, token) {
        ...
        if (callee.Name == 'COST' || callee.Name == 'WINNER' || callee.Name == 'PWINNER') {
            //筹码都需要换手率
            //下载流通股
            var item = {
                ID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_FINANCE,
                Args: [7],
                FunctionName: 'FINANCE',
                FunctionName2: callee.Name,
            };
            if (token) item.Token = { Index: token.Start, Line: token.LineNumber };
            this.FunctionData.push(item);
            return;
        ...
        if (callee.Name == 'STKINDI' || callee.Name == 'CALCSTOCKINDEX') {
            //指标调用
        ...
        while (true) {
            if (this.Match('.')) {
                ...
                expr = this.Finalize(
                    ...
                    this.Node.StaticMemberExpression(expr, property, startToken)
                );
            } else if (this.Match('(')) {
                this.Context.IsBindingElement = false;
                this.Context.IsAssignmentTarget = false;
...
function JSAlgorithm(errorHandler, symbolData) {
    ...
    this.REF = function (data, n) {
        ...
        if (typeof n == 'number') {
            if (IFrameSplitOperator.IsNumber(data)) {
                ...
                for (let i = 0; i < n; ++i) result.unshift(null);
            }
        } //n 为数组的情况
        else {
    ...
    */
    ...
    this.CalculateZIGLine = function (firstData, secondData, thridData, data, result) {
        ...
        var lineCache = {
            Start: { ID: firstData.ID, Value: firstData.Value },
            End: { ID: secondData.ID, Value: secondData.Value },
        };
        ...
        if (thridData.ID == data.length - 1) {
            ...
            //最后2个点的数据连成线
            lineCache = {
                Start: { ID: secondData.ID, Value: secondData.Value },
                End: { ID: thridData.ID, Value: thridData.Value },
            };
    ...
    */
    this.COST = function (data, node) {
        ...
        var kData = this.SymbolData.Data.Data;
        if (!kData || kData.length <= 0) return result;
        var aryCapital = this.SymbolData.GetStockCacheData({
            FunctionName: 'FINANCE',
            Args: [7],
            ArgCount: 1,
            Node: node,
    ...
    */
    this.LONGCROSS = function (data, data2, n) {
        ...
        for (let i = 0; i < count; ++i) {
            ...
            if (i >= data.length || i >= data2.length) continue;
            if (
                !this.IsNumber(data[i]) ||
                !this.IsNumber(data2[i]) ||
                !this.IsNumber(data[i - 1]) ||
                !this.IsNumber(data2[i - 1])
            )
                continue;
    ...
    */
    this.BETA = function (n) {
        ...
        for (let i = 0; i < stockData.Data.length; ++i) {
            ...
            if (stockItem.Close > 0 && stockItem.YClose > 0)
                stockProfit[i] = (stockItem.Close - stockItem.YClose) / stockItem.YClose;
            if (indexItem.Close > 0 && indexItem.YClose > 0)
        ...
        for (var i = 0, j = 0; i < stockData.Data.length; ++i) {
            result[i] = null;
            if (
                i >= stockProfit.length ||
                i >= indexProfit.length ||
                i >= averageStockProfit.length ||
                i >= averageIndexProfit.length
            )
                continue;
    ...
    */
    this.BETA2 = function (x, y, n) {
        ...
        for (var i = 0, j = 0; i < count; ++i) {
            ...
            if (i >= xProfit.length || i >= yProfit.length || i >= averageXProfit.length || i >= averageYProfit.length)
                continue;
    ...
    */
    ...
    this.STRCAT = function (str1, str2) {
        var result = [];
        if (this.IsString(str1) && this.IsString(str2)) result = str1 + str2;
        return result;
    };
    ...
    this.VARCAT = function (data, data2) {
        ...
        if (Array.isArray(data) && Array.isArray(data2)) {
            ...
            for (var i = 0; i < nCount; ++i) {
                ...
                if (i < data.length) {
                    var item = data[i];
                    if (this.IsString(item)) strValue += item;
                }
    ...
    this.STRSPACE = function (data) {
        ...
        if (Array.isArray(data)) {
            for (var i = 0; i < data.length; ++i) {
                result[i] = null;
                var item = data[i];
                if (TouchList.IsString(item)) result[i] = item + ' ';
            }
        } else {
            if (this.IsString(data)) result = data + ' ';
        }
    ...
    this.MOD = function (data, data2) {
        ...
        if (isNumber && isNumber2) {
            ...
            for (let i = 0; i < count; ++i) {
                ...
                if (i < data.length && i < data2.length) {
                    if (this.IsNumber(data[i]) && this.IsNumber(data2[i]))
                        result[i] = JSAlgorithm.MOD(data[i], data2[i]);
                }
            }
    ...
    */
    ...
    this.DRAWGBK_DIV = function (condition, color, color2, colorType, fillType) {
        ...
        if (
            ...
            !IFrameSplitOperator.IsNonEmptyArray(this.SymbolData.Data.Data)
        )
            return result;
...
function JSSymbolData(ast, option, jsExecute) {
    ...
    this.RecvLatestData = function (recvData) {
        ...
        if (IFrameSplitOperator.IsNumber(stock.increase))
            this.LatestData.set(DYNAINFO_ARGUMENT_ID.INCREASE, stock.increase);
        if (IFrameSplitOperator.IsNumber(stock.exchangerate))
            this.LatestData.set(DYNAINFO_ARGUMENT_ID.EXCHANGERATE, stock.exchangerate);
        if (IFrameSplitOperator.IsNumber(stock.amplitude))
            this.LatestData.set(DYNAINFO_ARGUMENT_ID.AMPLITUDE, stock.amplitude);
        ...
        for (var i = 0; i < data.data.length; ++i) {
            ...
            if (IFrameSplitOperator.IsNumber(item.value) || IFrameSplitOperator.IsString(item.value)) {
                JSConsole.Complier.Log(
                    `[JSSymbolData::RecvLatestDataVer2] symbol=${symbol} DYNAINFO(${item.id})=${item.value}.`
                );
        ...
        if (data.Ver == 2.0) {
            ...
            for (
                ...
                for (j = i, dayCount = 0; j >= 0 && dayCount < 5; --j, ++dayCount) {
                    ...
                    if (IFrameSplitOperator.IsNumber(item[6])) sumVol += item[6];
                }
                ...
                if (item.Type == Syntax.Literal) {
                    ...
                    if (!isFind) {
                        ...
                        this.Execute.ErrorHandler.ThrowError(
                            ...
                            token.Line,
                            0,
                            `${job.FunctionName}() Error: can't read ${item.Name}`
                        );
                    }
                }
            }
        ...
        if (this.DataType == HQ_DATA_TYPE.KLINE_ID && ChartData.IsDayPeriod(this.Period, true)) {
            ...
            JSNetwork.HttpRequest({
                ...
                data: {
                    ...
                    symbol: job.Symbol,
                    start: -1,
                    count: self.MaxRequestDataCount + 500, //多请求2年的数据 股票剔除停牌日期以后可以对上
                },
                method: 'POST',
                dataType: 'json',
                async: true,
    ...
    this.RecvIndexHistroyData = function (recvData) {
        ...
        if (JSCommonData.ChartData.IsDayPeriod(this.Period, false)) {
            //周期数据
    ...
    this.RecvIndexMinuteHistroyData = function (recvData) {
        ...
        if (JSCommonData.ChartData.IsMinutePeriod(this.Period, false)) {
            //周期数据
            let periodData = this.IndexData.GetPeriodData(this.Period);
            this.IndexData.Data = periodData;
        }
    ...
    this.GetIndexIncreaseCacheData = function (funcName, symbol, node) {
        var key;
        if (funcName == 'UPCOUNT')
            key = JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_INDEX_INCREASE_DATA.toString() + '-UpCount-' + symbol;
        else if (funcName == 'DOWNCOUNT')
            key = JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_INDEX_INCREASE_DATA.toString() + '-DownCount-' + symbol;

        if (!key || !this.ExtendData.has(key))
            this.Execute.ThrowUnexpectedNode(node, '不支持函数' + funcName + '(' + symbol + ')');
        ...
        if (JSCommonData.ChartData.IsDayPeriod(this.Period, true)) {
            ...
            if (this.NetworkFilter) {
                ...
                if (this.KLineDateTimeRange) {
                    ...
                    if (this.IsNumber(this.KLineDateTimeRange.End.Time))
                        obj.Request.KLineDataTimeRange.End.Time = this.KLineDateTimeRange.End.Time;
                }
    ...
    this.GetCurrBarsCount = function () {
        ...
        let result = [];
        for (let i = lCount - 1; i >= 0; --i) result.push(i);
    ...
    this.RecvMarginData = function (recvData, jobID) {
        ...
        var allData = [];
        if (
            jobID === JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARGIN_BALANCE ||
            jobID === JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_MARGIN_RATE
        ) {
    ...
    this.GetNewsAnalysisData = function (jobID) {
        ...
        var mapFolder = new Map([
            ...
            [JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_NEWS_ANALYSIS_BLOCKTRADING, 'Blocktrading'], //NEWS(7)   折价交易
        ...
        JSNetwork.HttpRequest({
            ...
            async: true,
            success: function (recvData) {
                if (recvData.statusCode == 200) self.RecvNewsAnalysisData(recvData, jobID);
                else self.RecvNewsAnalysisDataError(recvData, jobID);
                self.Execute.RunNextJob();
            },
        ...
        if (this.NetworkFilter) {
            ...
            var obj = {
                ...
                Request: {
                    Url: self.RealtimeApiUrl,
                    Type: 'POST',
                    Data: { id: lID, symbol: this.Symbol, daterange: dateRange },
                },
                Self: this,
                PreventDefault: false,
            };
    ...
    this.GetVariantData = function (jobItem) {
        ...
        var apiDownload;
        if (
            jobItem.VariantName == 'CAPITAL' ||
            jobItem.VariantName == 'TOTALCAPITAL' ||
            jobItem.VariantName == 'EXCHANGE'
        ) {
            ...
            jobItem.VariantName == 'HYBLOCK' ||
            jobItem.VariantName == 'DYBLOCK' ||
            jobItem.VariantName == 'GNBLOCK'
        ) {
    ...
    this.GetProFinance = function (jobItem) {
        ...
        if (
            [
                JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_GPJYVALUE,
                JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_SCJYVALUE,
                JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_BKJYVALUE,
            ].includes(jobID)
        ) {
        ...
        if (this.NetworkFilter) {
            ...
            var obj = {
                ...
                Request: {
                    ...
                    Type: 'POST',
                    Data: { Args: aryArgs, symbol: this.Symbol, daterange: dateRange },
                },
                Self: this,
                PreventDefault: false,
            };
            ...
            if (Array.isArray(recvData)) {
                ...
                if (key == 'EXCHANGE') {
                    ...
                    for (var i in result) {
                        ...
                        if (result[i] > 0) result[i] = (kitem.Vol / result[i]) * 100;
                    }
                }
    ...
    this.GetStockCacheData = function (obj) {
        ...
        else if (obj.CustomName) key = obj.CustomName; //自定义名字
        else return null;
    ...
    */
    this.SysParam = function (id, jsExec) {
        ...
        if (id == 2) {
            ...
            jsExec.IsUsePageData = true;
            if (!IFrameSplitOperator.IsNumber(this.DrawInfo.End) || !IFrameSplitOperator.IsNumber(this.DrawInfo.Start))
                return [];
    ...
    this.JobArgumentsToArray = function (job, lCount) {
        ...
        if (args.length != lCount) {
            ...
            this.Execute.ErrorHandler.ThrowError(
                ...
                token.Line,
                0,
                `${job.FunctionName}() Error: argument count error.`
            );
        }
            ...
            if (item.Type == Syntax.Literal) {
                ...
                if (!isFind) {
                    ...
                    this.Execute.ErrorHandler.ThrowError(
                        ...
                        token.Line,
                        0,
                        `LoadAPIData() Error: can't read ${item.Name}`
                    );
                }
            } else {
    ...
    this.CallMemberScriptIndex = function (job) {
        ...
        if (this.Execute.VarTable.has(objName)) {
            ...
            if (memberValue.hasOwnProperty(PropertyName)) {
                JSConsole.Complier.Log(
                    `[JSSymbolData::CallMemberScriptIndex] index data ${objName}.${PropertyName} in cache.`
                );
                return this.Execute.RunNextJob();
            }
        }
        ...
        if (!this.ReadIndexFunctionValue(callInfo, indexInfo)) {
            ...
            this.Execute.ErrorHandler.ThrowError(
                ...
                token.Line,
                0,
                `CallMemberScriptIndex() Error: '${callInfo}' ${indexInfo.Error}`
            );
        }
    ...
    this.CallDynamicScriptIndex = function (job, varTable) {
        ...
        if (!this.ReadIndexFunctionValue(callInfo, indexInfo)) {
            ...
            this.Execute.ErrorHandler.ThrowError(
                ...
                token.Line,
                0,
                `CallDynamicScriptIndex() Error: '${callInfo}' ${indexInfo.Error}`
            );
        }
        ...
        if (!systemItem) {
            ...
            this.Execute.ErrorHandler.ThrowError(
                ...
                token.Line,
                0,
                `CallDynamicScriptIndex() Error: '${callInfo}' ${indexInfo.Name} 指标不存在`
            );
        }
        ...
        if (!this.ReadDynamicIndexArgumentValue(job.Args, indexInfo, varTable)) {
            ...
            this.Execute.ErrorHandler.ThrowError(
                ...
                token.Line,
                0,
                `CallDynamicScriptIndex() ${indexInfo.Name} 指标参数错误 : ${indexInfo.Error} `
            );
        }
    ...
    this.ReadDynamicIndexArgumentValue = function (args, result, varTable) {
        ...
        for (
            var i = 0;
            i < result.SytemIndex.Args.length;
            ++i //复制参数
        ) {
            var item = result.SytemIndex.Args[i];
    ...
    *******************************************************************************************************************************/
    this.CallScriptIndex = function (job, varTable) {
        ...
        if (!job.Args || !(job.Args.length >= 2)) {
            ...
            this.Execute.ErrorHandler.ThrowError(
                ...
                token.Line,
                0,
                `CallScriptIndex() Error: ${job.FunctionName} 参数错误`
            );
        }
        ...
        if (!this.ReadSymbolArgumentValue(job.Args[0], indexInfo)) {
            ...
            this.Execute.ErrorHandler.ThrowError(
                ...
                token.Line,
                0,
                `CallScriptIndex() Error: ${indexInfo.Error}`
            );
        }
        ...
        if (job.FunctionName == 'CALCSTOCKINDEX') {
            if (!this.ReadIndexFunctionOut(job.Args[2], indexInfo)) {
                //读取返回值索引
                var token = job.Token;
                this.Execute.ErrorHandler.ThrowError(
                    token.Index,
        ...
        if (!systemItem) {
            ...
            this.Execute.ErrorHandler.ThrowError(
                ...
                token.Line,
                0,
                `CallScriptIndex() ${indexInfo.Name} 指标不存在`
            );
        }
    ...
    this.ExecuteScriptIndexError = function (error, indexInfo) {
        ...
        this.Execute.ErrorHandler.ThrowError(
            ...
            token.Line,
            0,
            `CallScriptIndex() ${indexInfo.Name} 指标执行错误 : ${error} `
        );
    };
    ...
    this.GetScriptIndexOutData = function (args, node, funcName) {
        ...
        if (funcName == 'CALCSTOCKINDEX') {
            if (!this.ReadIndexFunctionOut(args[2], indexInfo))
                //读取返回值索引
        ...
        var systemIndex = new JSIndexScript();
        var systemItem = systemIndex.Get(indexInfo.Name);
        if (!systemItem) this.Execute.ThrowUnexpectedNode(node, `${funcName}() 指标错误: ${indexInfo.Name} 指标不存在`);
    ...
    */
    this.TIME2 = function () {
        ...
        for (let i = 0; i < this.Data.Data.length; ++i) {
            var item = this.Data.Data[i];
            if (this.IsNumber(item.Time)) result[i] = item.Time * 100;
            else result[i] = 0;
        }
...
JSSymbolData.prototype.JsonDataToFinance = function (data) {
    ...
    for (let i = 1; i <= 4; ++i) {
        ...
        if (!finance || !announcement || !this.IsNumber(announcement.year) || !this.IsNumber(announcement.quarter))
            continue;
        if (financeData) {
...
var JS_EXECUTE_JOB_ID = {
    ...
    JOB_DOWNLOAD_NEWS_ANALYSIS_BLOCKTRADING: 2006, //折价交易
    ...
    GetNewsAnalysisID: function (value) {
        let dataMap = new Map([
            ...
            [7, JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_NEWS_ANALYSIS_BLOCKTRADING], //NEWS(7)   折价交易
...
var JS_ARRAY_PROFESSIONAL_FINANCE = [
    {
        Name: 'GPJYVALUE',
        JobID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_GPJYVALUE,
        Explain: '股票交易类数据GPJYVALUE(ID,N,TYPE)',
        FuncName: 'JSSymbolData::GetGPJYValue',
        ArgCount: 3,
    },
    {
        Name: 'GPJYONE',
        JobID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_GPJYONE,
        Explain: '股票交易类数据GPJYONE(ID,N,Y,MMDD)',
        FuncName: 'JSSymbolData::GetGPJYOne',
        ArgCount: 4,
    },
    {
        Name: 'SCJYVALUE',
        JobID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_SCJYVALUE,
        Explain: '市场交易类数据SCJYVALUE(ID,N,TYPE)',
        FuncName: 'JSSymbolData::GetSCJYValue',
        ArgCount: 3,
    },
    {
        Name: 'SCJYONE',
        JobID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_SCJYONE,
        Explain: '市场交易类数据SCJYONE(ID,N,Y,MMDD)',
        FuncName: 'JSSymbolData::GetSCJYOne',
        ArgCount: 4,
    },
    {
        Name: 'BKJYVALUE',
        JobID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_BKJYVALUE,
        Explain: '板块交易类数据BKJYVALUE(ID,N,TYPE)',
        FuncName: 'JSSymbolData::GetBKJYValue',
        ArgCount: 3,
    },
    {
        Name: 'BKJYONE',
        JobID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_BKJYONE,
        Explain: '板块交易类数据BKJYONE(ID,N,Y,MMDD)',
        FuncName: 'JSSymbolData::GetBKJYOne',
        ArgCount: 4,
    },
    {
        Name: 'FINVALUE',
        JobID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_FINVALUE,
        Explain: '财务数据FINVALUE(ID)',
        FuncName: 'JSSymbolData::GetFinValue',
        ArgCount: 1,
    },
    {
        Name: 'FINONE',
        JobID: JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_FINONE,
        Explain: '财务数据FINONE(ID,Y,MMDD)',
        FuncName: 'JSSymbolData::GetFinOne',
        ArgCount: 3,
    },
];
...
function JSExecute(ast, option) {
    ...
    this.RunNextJob = function () {
        ...
        switch (jobItem.ID) {
            ...
            case JS_EXECUTE_JOB_ID.JOB_DOWNLOAD_NEWS_ANALYSIS_BLOCKTRADING: //折价交易
    ...
    this.RunAST = function () {
        ...
        for (var i = 0; i < this.AST.Body.length; ++i) {
            ...
            if (item.Type == Syntax.ExpressionStatement && item.Expression) {
                if (
                    item.Expression.Type == Syntax.AssignmentExpression &&
                    ...
                    if (this.VarDrawTable.has(varName)) {
                        //绘图函数赋值
                ...
                } else if (item.Expression.Type == Syntax.MemberExpression) {
                    //MA.MA2
                    ...
                    for (var j = 0; j < item.Expression.Expression.length; ++j) {
                        ...
                        if (
                            itemExpression.Type == Syntax.AssignmentExpression &&
                            itemExpression.Operator == ':' &&
                            itemExpression.Left
                        ) {
                            if (j == 0) {
                            ...
                            else if (value.indexOf('NOTEXT') == 0 || value.indexOf('NOTITLE') == 0)
                                isShowTitle = false; //标题不显示
                        ...
                        } else if (itemExpression.Type == Syntax.MemberExpression) {
                            //"MA.MA2"(5,12,29), COLORRED;
                            if (j == 0) {
                    ...
                    if (pointDot && varName) {
                        ...
                        let value = {
                            Name: varName,
                            Data: outVar,
                            Radius: g_JSChartResource.CIRCLEDOT.Radius,
                            Type: 3,
                        };
                        if (color) value.Color = color;
                        if (lineWidth) value.LineWidth = lineWidth;
                        this.OutVarTable.push(value);
                    } else if (upDownDot && varName) {
                        //彩色点
                        let outVar = this.VarTable.get(varName);
                        if (!Array.isArray(outVar)) outVar = this.SingleDataToArrayData(outVar);
                        let value = {
    ...
    this.Run = function () {
        ...
        if (this.UpdateUICallback) {
            ...
            if (
                this.CallbackParam &&
                this.CallbackParam.Job &&
                this.CallbackParam.Job.ID == JS_EXECUTE_JOB_ID.JOB_EXECUTE_INDEX
            ) {
                this.UpdateUICallback(data, this.CallbackParam, this.SymbolData);
            } else {
                if (
                    this.CallbackParam &&
                    this.CallbackParam.Self &&
                    this.CallbackParam.Self.ClassName === 'ScriptIndexConsole'
                )
    ...
    this.VisitCallExpression = function (node) {
        ...
        for (let i = 0; i < node.Arguments.length; ++i) {
            ...
            let value;
            if (funcName === 'IFC' && i >= 1) break; //IFC先处理第1个条件参数
        ...
        if (g_JSComplierResource.IsCustomDataFunction(funcName)) {
            var functionInfo = g_JSComplierResource.CustomDataFunction.Data.get(funcName);
            node.Out = this.SymbolData.GetStockCacheData({
                FunctionName: funcName,
                Args: args,
                ArgCount: functionInfo.ArgCount,
                Node: node,
            });
            node.Draw = null;
            return node.Out;
    ...
    this.VisitAssignmentExpression = function (node) {
        ...
        else if (right.Type == Syntax.UnaryExpression) value = this.VisitUnaryExpression(right);

        if (JS_EXECUTE_DEBUG_LOG)
            JSConsole.Complier.Log('[JSExecute::VisitAssignmentExpression]', varName, ' = ', value);
        ...
        while (stack.length != 0) {
            ...
            if (temp.Left && node != temp.Left && node != temp.Right) {
                ...
                if (value.Type == Syntax.BinaryExpression) {
                    ...
                    if (JS_EXECUTE_DEBUG_LOG)
                        JSConsole.Complier.Log(
                            '[JSExecute::VisitBinaryExpression] BinaryExpression',
                            value,
                            leftValue,
                            rightValue
                        );
                    ...
                    if (JS_EXECUTE_DEBUG_LOG)
                        JSConsole.Complier.Log('[JSExecute::VisitBinaryExpression] BinaryExpression', value);
                    ...
                    if (JS_EXECUTE_DEBUG_LOG)
                        JSConsole.Complier.Log(
                            '[JSExecute::VisitBinaryExpression] LogicalExpression',
                            value,
                            leftValue,
                            rightValue
                        );
                    ...
                    if (JS_EXECUTE_DEBUG_LOG)
                        JSConsole.Complier.Log('[JSExecute::VisitBinaryExpression] LogicalExpression', value);
                }
...
export { JSCommonComplier, ErrorHandler, JSComplier, JSParser, Syntax, JS_EXECUTE_JOB_ID, g_JSComplierResource };
