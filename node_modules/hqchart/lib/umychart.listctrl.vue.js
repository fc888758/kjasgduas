...
function JsCellItem() {
    ...
    this.SetPercentageValue = function (value, option) {
        this.Value = value;
        if (JSTableHelper.IsNumber(value)) this.SetText(value.toFixed(option.Dec) + '%');else this.SetText(option.NullText);

        if (value > option.CompareValue) this.SetCSS(option.CSS[0]);else if (value < option.CompareValue) this.SetCSS(option.CSS[1]);else this.SetCSS(option.CSS[2]);
    };
    ...
    this.SetPriceValue = function (value, option) {
        ...
        this.Value = value;
        if (!JSTableHelper.IsNumber(value)) this.SetText(option.NullText);else this.SetText(value.toFixed(option.Dec));
...
JsHeadCellItem.GetCellRule = function (type) {
    var CELL_RULE_MAP = new _map2.default([[HEAD_CELL_TYPE_ID.CELL_STRING_ID, { Type: HEAD_CELL_TYPE_ID.CELL_STRING_ID, CSS: 'jshcell-string' }]]);
...
function JsHeadRowItem() {
    ...
    this.Build = function (index, headElement) {
        this.Index = index;
        if (index < headElement.rows.length) this.Element = headElement.rows[index];else this.Element = headElement.insertRow(index);
...
JSTableHelper.FormatValueString = function (value, floatPrecision, changedfloatPrecision, languageID) {
    ...
    if (absValue < 10000) return value.toFixed(floatPrecision);else if (absValue < 100000000) return (value / 10000).toFixed(changedfloatPrecision) + "万";else if (absValue < 1000000000000) return (value / 100000000).toFixed(changedfloatPrecision) + "亿";else return (value / 1000000000000).toFixed(changedfloatPrecision) + "万亿";
};
...
exports.default = {
    //类导出
    JSTable: JSTable,
    JsHeadCellItem: JsHeadCellItem,
    JsCellItem: JsCellItem,
    CELL_TYPE_ID: CELL_TYPE_ID,
    HEAD_CELL_TYPE_ID: HEAD_CELL_TYPE_ID
