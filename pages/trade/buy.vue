        ...
        <scroll-view class="" scroll-x="false" scroll-y="true" upper-threshold="50" lower-threshold="50">
            ...
            <view class="price-input-container">
                ...
                <view class="price-input-wrapper">
                    <text>￥</text>
                    <text style="font-size: 60rpx"> {{ paymentAmount }}</text>
                </view>
            </view>
            <!-- 手数 -->
    ...
    export default {
        ...
        computed: {
            ...
            discountPrice() {
                ...
                if (this.stockDetail && this.stockDetail.current) {
                    currentPrice = parseFloat(this.stockDetail.current);
                } else if (this.price && !isNaN(this.price) && this.price > 0) {
                    currentPrice = parseFloat(this.price);
                } else {
                    return 0.0; // 返回0而不是空字符串
                }
            ...
            // 计算每股总价
            totalPricePerShare() {
                if (!this.discountPrice || this.discountPrice === '' || parseFloat(this.discountPrice) <= 0) {
                    // 当手数为0时，仍然需要基于股票现价计算每股总价，用于比例选择
                    if (this.stockDetail && this.stockDetail.current) {
                        const currentPrice = parseFloat(this.stockDetail.current);
                        const discountPrice =
                            this.discountRate && !isNaN(this.discountRate)
                                ? currentPrice * (this.discountRate / 10)
                                : currentPrice;
                        const stampTax = discountPrice * (this.stampTaxRate || 0);
                        const commission = discountPrice * (this.commissionRate || 0);
                        return (discountPrice + stampTax + commission).toFixed(2);
                    }
                    return '';
                }
                return (
        ...
        methods: {
            ...
            updateActiveRatio() {
                ...
                const currentShares = this.quantity * 100; // 当前手数转换为股数

                // 计算各个比例对应的股数
                const fullShares = Math.floor(baseShares / 100) * 100;
                const halfShares = Math.floor(baseShares / 2 / 100) * 100;
                const thirdShares = Math.floor(baseShares / 3 / 100) * 100;
                const quarterShares = Math.floor(baseShares / 4 / 100) * 100;

                // 允许一定的误差范围（±1手）
                const tolerance = 100;

                if (Math.abs(currentShares - fullShares) <= tolerance) {
                    this.activeRatio = 'full';
                } else if (Math.abs(currentShares - halfShares) <= tolerance) {
                    this.activeRatio = 'half';
                } else if (Math.abs(currentShares - thirdShares) <= tolerance) {
                    this.activeRatio = 'third';
                } else if (Math.abs(currentShares - quarterShares) <= tolerance) {
                    this.activeRatio = 'quarter';
                } else {
                    this.activeRatio = '';
                }
            },
            ...
            selectRatio(ratio) {
                this.$nextTick(() => {
                    ...
                    if (!this.totalPricePerShare || parseFloat(this.totalPricePerShare) <= 0 || !this.userInfo.money) {
                        return;
                    }
                    ...
                    // 计算手数（1手=100股）并使用 $set  Vue 能检测到变化
            ...
            async confirmOrder() {
                ...
                switch (this.type) {
                    case 'bd': // 要约购
                        const isBdPassword = await this.$api.passwordDetection({ type: 3, id: this.options.bd_id }); //检测是否要密码
                        const orderBdParams = {
            ...
            async confirmPassword() {
                ...
                try {
                    ...
                    const result = await this.$api.isPasswordCorrect({
                        ...
                        id: this.type == 'bd' ? this.options.bd_id : this.stockId,
                    });
