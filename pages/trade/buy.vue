        ...
        <scroll-view class="" scroll-x="false" scroll-y="true" upper-threshold="50" lower-threshold="50">
            ...
            <view class="stock-input-container">
                ...
                <view>
                    ...
                    <view>
                        <text
                            :style="{ color: stockDetail.chg && stockDetail.chg.includes('-') ? '#07c160' : '#fb4e50' }"
                            >{{ stockDetail.chg || 0 }}</text
                        >
                        <text
                            :style="{ color: stockDetail.per_chg && stockDetail.per_chg < 0 ? '#07c160' : '#fb4e50' }"
                        >
            ...
            <view class="quantity-input-container">
                ...
                <text style="font-size: 22rpx; color: #999">(1手=100股)</text>
            </view>
            ...
            <view class="order-book">
                <view class="book-content">
                    <view class="sell-orders">
                        <view
                            class="order-item"
                            v-if="stockDetail.sell_price_5"
                            @click="setPrice(stockDetail.sell_price_5)"
                        >
                        ...
                        <view
                            class="order-item"
                            v-if="stockDetail.sell_price_4"
                            @click="setPrice(stockDetail.sell_price_4)"
                        >
                        ...
                        <view
                            class="order-item"
                            v-if="stockDetail.sell_price_3"
                            @click="setPrice(stockDetail.sell_price_3)"
                        >
                        ...
                        <view
                            class="order-item"
                            v-if="stockDetail.sell_price_2"
                            @click="setPrice(stockDetail.sell_price_2)"
                        >
                        ...
                        <view
                            class="order-item"
                            v-if="stockDetail.sell_price_1"
                            @click="setPrice(stockDetail.sell_price_1)"
                        >
                    ...
                    <view class="buy-orders">
                        <view
                            class="order-item"
                            v-if="stockDetail.buy_price_1"
                            @click="setPrice(stockDetail.buy_price_1)"
                        >
                        ...
                        <view
                            class="order-item"
                            v-if="stockDetail.buy_price_2"
                            @click="setPrice(stockDetail.buy_price_2)"
                        >
                        ...
                        <view
                            class="order-item"
                            v-if="stockDetail.buy_price_3"
                            @click="setPrice(stockDetail.buy_price_3)"
                        >
                        ...
                        <view
                            class="order-item"
                            v-if="stockDetail.buy_price_4"
                            @click="setPrice(stockDetail.buy_price_4)"
                        >
                        ...
                        <view
                            class="order-item"
                            v-if="stockDetail.buy_price_5"
                            @click="setPrice(stockDetail.buy_price_5)"
                        >
            ...
            <view class="trade-labels">
                <view class="buy-label">
                    <text>跌停价：{{ stockDetail.l_d_price || '--' }}</text>
                </view>
                <view class="sell-label">
                    <text>涨停价：{{ stockDetail.l_u_price || '--' }}</text>
                </view>
            </view>
            ...
            <view class="trade-info">
                ...
                <view class="info-item">
                    <text class="info-label">可买手数</text>
                    <text class="info-value">{{ maxBuyableLots }}</text>
                </view>
                <view class="info-item">
                    <text class="info-label">支付金额</text>
                    <text class="info-value">¥ {{ paymentAmount }}</text>
                </view>
            </view>
            ...
            <view class="popup-content">
                ...
                <view class="popup-info">
                    ...
                    <view class="popup-item">
                        ...
                        <text class="popup-value">¥ {{ availableBalance }}</text>
                    </view>
                    <view class="popup-item">
                        <text class="popup-label">可买手数</text>
                        <text class="popup-value">{{ maxBuyableLots }}</text>
                    </view>
    ...
    export default {
        ...
        computed: {
            ...
            maxBuyableShares() {
                ...
                // 可用余额除以每股总价
                const maxShares = (this.availableBalance / parseFloat(this.totalPricePerShare)).toFixed(2);
                // 股票交易以100股为单位
        ...
        methods: {
            init() {
                this.getStockDetail(this.stockId);
                this.getUserInfo();
                this.getCommission();
            },
            // 获取股票详情
            async getStockDetail(stock_id) {
                try {
                    this.stockDetail = await this.$api.stockDetail({ stock_id });
                    // 获取股票详情后，自动使用现价
                    if (this.stockDetail && this.stockDetail.current) {
                        this.price = this.quantity && this.stockDetail.current;
                    }
                } catch (error) {
                    console.log(error);
                }
            ...
            async getUserInfo() {
                try {
                    ...
                    if (result) {
                        ...
                        this.availableBalance = parseFloat(result.money || 0).toFixed(2);
                    }
                } catch (error) {
            ...
            async confirmOrder() {
                ...
                switch (this.type) {
                    case 'bd': // 要约购
                        const isBdPassword = await this.$api.passwordDetection({ type: 3, id: this.options.bdID }); //检测是否要密码
                        const orderBdParams = {
                        ...
                        if (isBdPassword) {
                            // 需要密码，显示密码弹框
                            this.passwordOrderParams = orderBdParams;
                            this.showPasswordPopup = true;
                            this.$modal.closeLoading();
                        } else {
                            // 不需要密码，直接提交订单
                            this.submitBlockTradeOrder(orderBdParams);
                        }
                        break;
                    case 'ftc': // 涨停下单
                        const isFtcPassword = await this.$api.passwordDetection({ type: 2, id: this.stockId }); //检测是否要密码
                        const orderFtcParams = {
                        ...
                        if (isFtcPassword) {
                            // 需要密码，显示密码弹框
                            this.passwordOrderParams = orderFtcParams;
                            this.showPasswordPopup = true;
                            this.$modal.closeLoading();
                        } else {
                            // 不需要密码，直接提交订单
                            this.submitVipTradeOrder(orderFtcParams);
                        }
                        break;
            ...
            async confirmPassword() {
                ...
                try {
                    ...
                    const result = await this.$api.isPasswordCorrect({
                        ...
                        type: this.type == 'bd' ? 3 : 2,
                        id: this.type == 'bd' ? this.options.bdID : this.stockId,
                    });
                    ...
                    if (result) {
                        ...
                        if (this.type === 'bd' && this.passwordOrderParams) {
                            ...
                            // 提交VIP调研订单
                            this.submitVipTradeOrder(this.passwordOrderParams, this.password);
                        } else {
                            // 提交普通订单
            ...
            // 提交VIP调研订单
            submitVipTradeOrder(params, password = null) {
                // 如果有密码，添加到参数中
                if (password) {
                    params.password = password;
                }
...
<style lang="scss" scoped>
    ...
    .buy-page scroll-view {
        padding-bottom: 130rpx; /* 根据底部操作区的高度调整 */
    }
    ...
    .order-book {
        ...
        .book-content {
            ...
            .sell-orders .order-item .price-value {
                color: #07c160; /* 卖盘价格使用绿色 */
            }

            .buy-orders .order-item .price-value {
                color: #fb4e50; /* 买盘价格使用红色 */
            }
        }
    }
